...
export class JenkinsBasicBuildStatisticsService implements StatisticsEntryProvider {
    ...
    private getAverageBuildDuration(builds: Map<IJenkinsJob, Array<IJenkinsBuild>>): StatisticsEntry {
        ...
        let averageDurationTimestamp = Math.round(totalDurationTimestamp / this.util.mapToArray(builds).length);
    ...
    private getBuildsAfterDate(builds: Map<IJenkinsJob, Array<IJenkinsBuild>>, afterDate: Date): Array<IJenkinsBuild> {
        ...
        return buildArray.filter(build => this.util.isSameOrAfterDate(new Date(build.timestamp), afterDate));
    }
}
