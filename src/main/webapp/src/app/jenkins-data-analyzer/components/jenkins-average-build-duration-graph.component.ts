...
@Component({
    selector: 'jenkins-average-build-duration-graph',
    templateUrl: '../templates/jenkins-average-build-duration-graph.template.html'
})
export class JenkinsAverageBuildDurationGraphComponent implements OnInit {
    ...
    BUILD_GREEN_THRESHOLD = 10;
    BUILD_YELLOW_THRESHOLD = 20;
    ...
    private getJobsData(data: IJenkinsData): Array<any> {
        ...
        let averageWeightedDuration: number = parent.utilService.getBuildAverageWeightedDuration(parent.utilService.mapToArray(data.builds));
