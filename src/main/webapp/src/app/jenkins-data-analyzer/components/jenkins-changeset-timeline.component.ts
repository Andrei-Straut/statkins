...
@Component({
    ...
    templateUrl: '../templates/jenkins-changeset-timeline.template.html'
})
export class JenkinsChangeSetTimelineComponent implements OnInit {
    ...
    groupVisibility: VisBuildTimelineBuildVisibility = {
        isFullscreen: false,
        stack: true,
        success: true,
        unstable: true,
        ...
        queueTimes: undefined,
        buildWithQTime: undefined,
        buildWithoutQTime: undefined,
    };
    ...
    toggleFullscreen() {
        ...
        if (!this.groupVisibility.isFullscreen) {
    ...
    private isToBeIncluded(changeSet: IJenkinsChangeSet): boolean {
        ...
        if (!this.utilService.isSameOrAfterDate(new Date(changeSet.timestamp), lastWeek)) {
            return false;
        }
